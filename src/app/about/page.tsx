...
const TIMELINE = [
  ...
  },
  {
    ...
    body: "Units deploy into humanitarian, industrial, and military settings across a wide spread of temperature and humidity conditions.",
  },
  {
    year: "Today",
    title: "35+ countries, five continents",
    body: "The range covers everything from a household dispenser to a containerised emergency unit, with a common process behind all of it.",
  },
  {
    year: "Now",
...
export default function AboutPage() {
  return (
    <>
      <PageHero
        eyebrow="About VidaTech"
        title={
          <>
            Local roots.{" "}
            <span className="text-teal">Global technology.</span>
          </>
        }
        lede="VidaTech is the exclusive Pakistani partner of GENAQ, the Spanish pioneer that has been engineering atmospheric water generators since 2008 — now trusted in more than 35 countries across five continents. From homes and offices to industry, agriculture, and emergency response, we bring water security within reach."
      />

      <section className="section-y">
        <div className="shell">
          <div className="grid gap-px overflow-hidden rounded-3xl border border-blue/30 bg-blue/25 md:grid-cols-3">
            {PILLARS.map((p, i) => (
              <Reveal key={p.kind} delay={i * 0.08} className="bg-paper p-8">
            ...
            {TIMELINE.map((entry, i) => (
              <Reveal as="li" key={entry.year} delay={i * 0.08}>
                <div className="flex h-full flex-col">
                  {/* The rule fills further along the sequence */}
                  <div
      ...
      <WaveDivider from="var(--color-blue-50)" to="var(--color-paper)" flip />
